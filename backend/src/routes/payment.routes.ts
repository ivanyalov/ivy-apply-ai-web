...
import { getSubscriptionByUserId, updateSubscription, createSubscription } from '../models/Subscription';
...
router.post('/cloudpayments/notify', async (req, res) => {
  try {
    ...
    if (Status === 'Completed') {
      ...
      console.log(`Payment processed successfully for user ${userId}, subscription ${subscription.id}`);
...
router.post('/cloudpayments/create-subscription', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://api.cloudpayments.ru/subscriptions/create', {
      token,
      accountId,
      description,
      email,
      ...
      headers: {
        ...
        'Authorization': `Basic ${Buffer.from(`${process.env.CLOUDPAYMENTS_PUBLIC_ID}:${process.env.CLOUDPAYMENTS_SECRET_KEY}`).toString('base64')}`
      }
    });
...
router.post('/cloudpayments/test', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://api.cloudpayments.ru/test', {}, {
      headers: {
        'Content-Type': 'application/json',
...
router.post('/cloudpayments/cancel-subscription', async (req, res) => {
  try {
    ...
    console.error('CloudPayments cancel subscription error:', error.response?.data || error.message);
    if (error.response?.data) {
      res.status(400).json(error.response.data);
    } else {
      res.status(500).json({
...
router.post('/cloudpayments/get-subscription', async (req, res) => {
  try {
    ...
    const response = await axios.post('https://api.cloudpayments.ru/subscriptions/get', {
      Id: subscriptionId
    }, {
      headers: {
        'Content-Type': 'application/json',
