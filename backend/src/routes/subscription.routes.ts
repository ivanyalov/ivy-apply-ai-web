...
router.post('/signup-trial', (req, res) => {
    ...
    subscriptionService.signupAndStartTrial(email, password)
        .then(({ user, subscription }) => {
            const authSvc = new AuthService();
            const token = authSvc.verifyToken(user.id);
...
router.post('/start-trial', authMiddleware, (req, res) => {
    ...
    subscriptionService.startTrial(userId)
        ...
        .catch(error => {
            if (error instanceof Error && (error.message === 'User already has an active subscription or trial.')) {
